...
/****** Added By Y.Born ******/
extern unsigned  peer_pid; // Peer PID
...
int main(int argc, char **argv)
{
    ...
    int fd = open(OPEN_FILE, O_RDWR);
    if((num = read(fd, pid, sizeof(pid))) == -1) {
        printf("Failed to open dpdk-ovs.pid\n");
        return -1;
    } else {
        peer_pid = atol(pid);
//         peer = 0;
//         rth.peer.nl_pid = atol(pid);
    }

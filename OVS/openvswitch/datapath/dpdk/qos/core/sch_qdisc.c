...
struct Qdisc * qdisc_alloc(struct vport_info *vport, struct Qdisc_ops *ops) {
    ...
    unsigned int size = QDISC_ALIGN(sizeof(struct Qdisc)) + ops->priv_size;

    sch = (struct Qdisc *) rte_zmalloc(name, size, CACHE_LINE_SIZE);
    if(!sch)
        goto errout;

    INIT_LIST_HEAD(&sch->list);
//    sch->q  = q_create(name, RING_F_SP_ENQ | RING_F_SC_DEQ);
...
//      stab = kmalloc(sizeof(*stab) + tsize * sizeof(u16), GFP_KERNEL);
        stab = rte_zmalloc(NULL, sizeof(*stab) + tsize * sizeof(u16), 0);
        if (!stab) {
...
//       }
        ...
        sch->handle = handle;
        RTE_LOG(DEBUG, QoS, "%s: Qdisc.handle is: %u\n", __func__, handle);
...
//        cond_resched();
        ...
        if(q->qlen != rte_ring_count(q->q)) {
            RTE_LOG(DEBUG, QoS, "qlen: %d, ring_count: %d\n", q->qlen, rte_ring_count(q->q));
        }

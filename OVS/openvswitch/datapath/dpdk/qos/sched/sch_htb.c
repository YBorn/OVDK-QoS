...
//        struct work_struct      work;
        ...
        struct rte_ring         *direct_queue;
        long                    direct_queue_len;
        long                    direct_pkts;
...
static struct htb_class *htb_classify(struct rte_mbuf *pkt, struct Qdisc *sch,
...
//                tcf = cl->filter_list;
//        }
        u32 classid = pkt->pkt.hash.sched + sch->handle;
        cl = htb_find(classid, sch);
        if(cl)
            return cl;
...
{
        WARN_ON(cl->level || !cl->un.leaf.q || !cl->un.leaf.q->qlen);
...
static int htb_enqueue(struct rte_mbuf *pkt, struct Qdisc *sch)
{
//        int uninitialized_var(ret);
        int ret;
        struct htb_sched *q = qdisc_priv(sch);
        struct htb_class *cl = htb_classify(pkt, sch, &ret);

        if (cl == HTB_DIRECT) {
                /* enqueue to helper queue */
                if (q->direct_queue_len < q->direct_qlen) {
                        rte_ring_enqueue(q->direct_queue, pkt);
                        q->direct_queue_len++; 
                        q->direct_pkts++;
                } else {
                ...
                return ret;
#endif
        } else if ((ret = qdisc_enqueue(pkt, cl->un.leaf.q)) != NET_XMIT_SUCCESS) {
...
static void htb_charge_class(struct htb_sched *q, struct htb_class *cl,
                             int level, struct rte_mbuf *pkt)
{
        int bytes = qdisc_pkt_len(pkt);
...
//        while (time_before(jiffies, stop_at)) {
        while (time_before(get_jiffies(), stop_at)) {
                struct htb_class *cl;
                s64 diff;
 ...
 */
static struct rte_mbuf *htb_dequeue_tree(struct htb_sched *q, const int prio,
                                        const int level)
{
...
next:
                 ...
                 */
                if (unlikely(cl->un.leaf.q->qlen == 0)) {
                ...
                pkt = cl->un.leaf.q->dequeue(cl->un.leaf.q);
                if (likely(pkt != NULL))
                        break;
        ...
        if (likely(pkt != NULL)) {
                bstats_update(&cl->bstats, pkt);
                cl->un.leaf.deficit[level] -= qdisc_pkt_len(pkt);
                 ...
                 */
                if (!cl->un.leaf.q->qlen)
                        htb_deactivate(q, cl);
                htb_charge_class(q, cl, level, pkt);
        }
        return pkt;
}

static struct rte_mbuf *htb_dequeue(struct Qdisc *sch)
{
        struct rte_mbuf *pkt = NULL;
        struct htb_sched *q = qdisc_priv(sch);
        int level;
...
//        skb = __skb_dequeue(&q->direct_queue);
        int ret = rte_ring_dequeue(q->direct_queue, (void **)&pkt);
        if(likely(ret == 0)) {
            q->direct_queue_len--;
        }
...
//        printf("jiffies: %lu\n", start_at);
        ...
        for (level = 0; level < TC_HTB_MAXDEPTH; level++) {
                ...
                while (m != (int)(-1)) {
                        int prio = ffz(m);
                        m |= 1 << prio;
                        pkt = htb_dequeue_tree(q, prio, level);
                        if (likely(pkt != NULL))
                                goto ok;
                }
        }
...
//        q->direct_queue = q_create(NULL, RING_F_SP_ENQ | RING_F_SC_DEQ);
        char name[32];
        static int index = 0;
        rte_snprintf(name, 32, "%s_direct_queue_%d", sch->vport->name, index);
        q->direct_queue = q_create(name, 0);
        index++;

        if (tb[TCA_HTB_DIRECT_QLEN])
                q->direct_qlen = nla_get_u32(tb[TCA_HTB_DIRECT_QLEN]);
        else {
                q->direct_qlen = QUEUE_RINGSIZE;
...
static int htb_dump(struct Qdisc *sch, struct sk_buf *skb)
{
        struct htb_sched *q = qdisc_priv(sch);
        struct nlattr *nest;
        struct tc_htb_glob gopt;
...
static int htb_dump_class(struct Qdisc *sch __rte_unused, unsigned long arg,
...
static int
htb_dump_class_stats(struct Qdisc *sch __rte_unused, unsigned long arg, struct gnet_dump *d)
{
        struct htb_class *cl = (struct htb_class *)arg;

        if (!cl->level && cl->un.leaf.q)
                cl->qstats.qlen = cl->un.leaf.q->qlen;
                     ...
                     struct Qdisc **old)
{
        ...
        if (cl->level)
                return -EINVAL;
        if (new == NULL &&
            (new = qdisc_create_dflt(sch->vport, &pfifo_qdisc_ops,
...
//        sch_tree_lock(sch);
        *old = cl->un.leaf.q;
        cl->un.leaf.q = new;
        if (*old != NULL) {
                qdisc_tree_decrease_qlen(*old, (*old)->qlen);
                qdisc_reset(*old);
        }
//        sch_tree_unlock(sch);
        return 0;
...
static struct Qdisc *htb_leaf(struct Qdisc *sch __rte_unused, unsigned long arg)
{
        struct htb_class *cl = (struct htb_class *)arg;
...
static void htb_destroy_class(struct Qdisc *sch __rte_unused, struct htb_class *cl)
{
        if (!cl->level) {
...
//        if (cl->children || cl->filter_cnt)
        ...
        if (!cl->level && htb_parent_last_child(cl)) {
                new_q = qdisc_create_dflt(sch->vport, &pfifo_qdisc_ops,
                            ...
                            unsigned long *arg)
{
        ...
        if (!cl) {              /* new class */
                ...
                err = -ENOBUFS;
                cl = rte_zmalloc(NULL, sizeof(*cl), 0);
                if (!cl)
                        goto failure;
...
//                                                tca[TCA_RATE] ? : &est.nla);
                 ...
                 */
                new_q = qdisc_create_dflt(sch->vport,
                                          &pfifo_qdisc_ops, classid);
//                sch_tree_lock(sch);
                if (parent && !parent->level) {
                        unsigned int qlen = parent->un.leaf.q->qlen;
